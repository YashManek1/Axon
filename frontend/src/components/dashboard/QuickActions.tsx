...
const actions = [
  {
    icon: Plus,
    label: "Deploy Job",
    desc: "Launch new automated task",
        ...
        <div className="grid grid-cols-2 gap-3">
          {actions.map((action) => (
            <button
              key={action.label}
              onClick={() => {
                if (action.actionType === "modal") setShowCreateJob(true);
                else if (action.toastMsg)
                  toast.info(action.label, action.toastMsg);
              }}
      ...
      {showCreateJob && (
        <CreateJobModal
          ...
          onSuccess={() => {
            ...
            // We could trigger a global re-fetch here if needed
          }}
        />
      )}
    </>
